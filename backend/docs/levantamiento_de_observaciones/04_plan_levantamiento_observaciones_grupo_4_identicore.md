...
| **PROPUESTA / PENDIENTE DE EJECUCIÓN** | Se requieren validaciones de formato estricto para DNI (8 dígitos) y RUC (11 dígitos válidos). Estructura implementada en SQL; ejecución H4 pendiente. | Segundo | `03_esquema_sigd_auth_v2.sql` - restricciones `CHECK chk_persona_natural_dni_format` y `chk_persona_ruc_format` |
| **PENDIENTE / LÓGICA DE APLICACIÓN** | Se requiere Argon2id efectivo y gestión de `sesion_usuario` con refresh tokens rotativos, control de reuso y auditoría IP/User-Agent. BD solo aporta almacenamiento. | Segundo | `03_esquema_sigd_auth_v2.sql` y `04_validacion_identicore_v2.md` |
| **PENDIENTE (cumplimiento jurídico)** | Se requiere modelar `consentimiento_datos` con historial de revocación inmutable bajo la Ley N.° 29733. Tabla existe; inmutabilidad y validación legal pendientes. | Jair / Segundo | Modelo, DDL y validación v2.0 |
