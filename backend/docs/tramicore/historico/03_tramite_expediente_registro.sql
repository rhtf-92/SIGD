...
-- [BORRADOR - Sujeto a cambios] Script SQL de Registro y ExpedientaciA|n
...
-- 1. LIMPIEZA / PREPARACIAôN DE ENTORNO
-- ADVERTENCIA: los DROP TABLE ... CASCADE (y DROP SEQUENCE) de este bloque son
-- A║nicamente para una base de datos aislada de pruebas. En un entorno real de
-- producciA|n NO deben ejecutarse, ya que eliminarA¡an datos e historial.
...
-- 2. SECUENCIA EXPLAìCITA PARA NUMERO_REGISTRO EN EL LIBRO GENERAL
CREATE SEQUENCE seq_asiento_numero_registro
    START WITH 10001
    INCREMENT BY 1
    NO MAXVALUE;
-- Nota: las secuencias de PostgreSQL garantizan unicidad y monotonicidad, pero
-- NO garantizan ausencia de huecos: si una transacciA|n consume nextval() y luego
...
CREATE TABLE tramite (
    ...
    codigo_tramite VARCHAR(20) NOT NULL UNIQUE, -- Ajustado segA║n diccionario: VARCHAR(20) NOT NULL
    asunto VARCHAR(500) NOT NULL,
    estado VARCHAR(30) NOT NULL DEFAULT 'REGISTRADO',
    fk_remitente BIGINT NOT NULL,    -- Nota: Llave forÃnea lA|gica hacia Personas/Usuarios (Grupo 4). Cubre usuarios registrados y solicitantes externos registrados de forma asistida sin credenciales.
    fk_destinatario BIGINT NULL,     -- Nota: Llave forÃnea lA|gica hacia Aüreas (Grupo 3). No se define FK fA¡sica por dependencia de mA|dulo externo.
...
CREATE TABLE asiento_registro (
    ...
    numero_registro BIGINT NOT NULL UNIQUE DEFAULT nextval('seq_asiento_numero_registro'), -- El formato visual con ceros (ej. 00000001) se gestionarÃ en el backend/app mediante LPAD()
    ...
    fk_destinatario BIGINT NULL,     -- Nota: Dependencia con Unidad OrgÃnica (Grupo 3)
...
-- Aìndices de bA║squeda y optimizaciA|n
...
('TRM-2026-0001', 'Solicitud de ExpediciA|n de TA¡tulo Profesional', 'EN_TRAMITE', 101, 301),
('TRM-2026-0002', 'RectificaciA|n de Notas de Asignatura de Base de Datos', 'OBSERVADO', 102, 302),
...
('MESA_VIRTUAL', 'Solicitud de ExpediciA|n de TA¡tulo Profesional', 1, 101, 301, FALSE, NULL),
('MESA_PRESENCIAL', 'RectificaciA|n de Notas de Asignatura de Base de Datos', 2, 102, 302, FALSE, NULL),
