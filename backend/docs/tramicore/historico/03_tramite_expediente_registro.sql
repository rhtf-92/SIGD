...
-- [BORRADOR - Sujeto a cambios] Script SQL de Registro y Expedientaci├│n
...
-- 1. LIMPIEZA / PREPARACI├ôN DE ENTORNO
-- ADVERTENCIA: los DROP TABLE ... CASCADE (y DROP SEQUENCE) de este bloque son
-- ├║nicamente para una base de datos aislada de pruebas. En un entorno real de
-- producci├│n NO deben ejecutarse, ya que eliminar├¡an datos e historial.
...
-- 2. SECUENCIA EXPL├ìCITA PARA NUMERO_REGISTRO EN EL LIBRO GENERAL
CREATE SEQUENCE seq_asiento_numero_registro
    START WITH 10001
    INCREMENT BY 1
    NO MAXVALUE;
-- Nota: las secuencias de PostgreSQL garantizan unicidad y monotonicidad, pero
-- NO garantizan ausencia de huecos: si una transacci├│n consume nextval() y luego
...
CREATE TABLE tramite (
    ...
    codigo_tramite VARCHAR(20) NOT NULL UNIQUE, -- Ajustado seg├║n diccionario: VARCHAR(20) NOT NULL
    asunto VARCHAR(500) NOT NULL,
    estado VARCHAR(30) NOT NULL DEFAULT 'REGISTRADO',
    fk_remitente BIGINT NOT NULL,    -- Nota: Llave for├ínea l├│gica hacia Personas/Usuarios (Grupo 4). Cubre usuarios registrados y solicitantes externos registrados de forma asistida sin credenciales.
    fk_destinatario BIGINT NULL,     -- Nota: Llave for├ínea l├│gica hacia ├üreas (Grupo 3). No se define FK f├¡sica por dependencia de m├│dulo externo.
...
CREATE TABLE asiento_registro (
    ...
    numero_registro BIGINT NOT NULL UNIQUE DEFAULT nextval('seq_asiento_numero_registro'), -- El formato visual con ceros (ej. 00000001) se gestionar├í en el backend/app mediante LPAD()
    ...
    fk_destinatario BIGINT NULL,     -- Nota: Dependencia con Unidad Org├ínica (Grupo 3)
...
-- ├ìndices de b├║squeda y optimizaci├│n
...
('TRM-2026-0001', 'Solicitud de Expedici├│n de T├¡tulo Profesional', 'EN_TRAMITE', 101, 301),
('TRM-2026-0002', 'Rectificaci├│n de Notas de Asignatura de Base de Datos', 'OBSERVADO', 102, 302),
...
('MESA_VIRTUAL', 'Solicitud de Expedici├│n de T├¡tulo Profesional', 1, 101, 301, FALSE, NULL),
('MESA_PRESENCIAL', 'Rectificaci├│n de Notas de Asignatura de Base de Datos', 2, 102, 302, FALSE, NULL),
