-- *Correcciones de auditoría aplicadas en v6.3.*
...
-- *6. EXPEDIENTE_FORMULARIO_RESPUESTA*
-- *Respuestas del formulario asociadas a la versión exacta utilizada.*
-- *============================================================================*
CREATE TABLE IF NOT EXISTS sigd_doc.expediente_formulario_respuesta (
    id_expediente_formulario_respuesta UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    id_expediente UUID NOT NULL,
    id_formulario_version UUID NOT NULL,
    payload_respuestas JSONB NOT NULL,
    fecha_creacion TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_efr_expediente UNIQUE (id_expediente),
    CONSTRAINT fk_efr_expediente FOREIGN KEY (id_expediente)
        REFERENCES sigd_doc.expediente(id_expediente) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT fk_efr_formulario_version FOREIGN KEY (id_formulario_version)
        REFERENCES sigd_doc.formulario_version(id_formulario_version) ON DELETE RESTRICT ON UPDATE RESTRICT,
    CONSTRAINT ck_efr_payload_objeto CHECK (jsonb_typeof(payload_respuestas) = 'object')
);

CREATE INDEX IF NOT EXISTS idx_efr_payload_gin
ON sigd_doc.expediente_formulario_respuesta
USING GIN (payload_respuestas jsonb_path_ops);

CREATE OR REPLACE FUNCTION sigd_doc.fn_validar_expediente_formulario_respuesta()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE v_formulario_version UUID;
BEGIN
    SELECT e.id_formulario_version INTO v_formulario_version
    FROM sigd_doc.expediente e WHERE e.id_expediente = NEW.id_expediente;
    IF NOT FOUND THEN RAISE EXCEPTION 'El expediente % no existe.', NEW.id_expediente; END IF;
    IF NEW.id_formulario_version <> v_formulario_version THEN
        RAISE EXCEPTION 'La respuesta debe corresponder a la versión exacta del formulario del expediente.';
    END IF;
    RETURN NEW;
END; $$;

DROP TRIGGER IF EXISTS tr_validar_expediente_formulario_respuesta
ON sigd_doc.expediente_formulario_respuesta;
CREATE TRIGGER tr_validar_expediente_formulario_respuesta
BEFORE INSERT OR UPDATE ON sigd_doc.expediente_formulario_respuesta
FOR EACH ROW EXECUTE FUNCTION sigd_doc.fn_validar_expediente_formulario_respuesta();
...
-- *CORRECCIÓN v6.2: se retiró el CHECK ck_documento_alta_cargado que existía*

-- *en la versión auditada. Ese CHECK exigía estado_documento='CARGADO' sin*

-- *distinguir INSERT de UPDATE, por lo que ninguna fila podía transicionar*

-- *jamás a OBSERVADO/APROBADO/REEMPLAZADO/ELIMINADO. La regla de que el alta*

-- *nace en CARGADO sigue vigente, pero solo mediante el trigger*

-- *fn_proteger_documento_aprobado (sección 19), que sí distingue TG_OP.*
...
-- *CORRECCIÓN v6.2: la deduplicación por SHA-256 (RN-ADJ-005) pasa de ser*

-- *única a nivel global a ser única por requisito. La regla de negocio*

-- *permite el mismo contenido binario en requisitos distintos bajo*

-- *confirmación; la unicidad global (uq_documento_sha256 sobre sha256_hash*

-- *solo) lo impedía indebidamente.*

CREATE UNIQUE INDEX IF NOT EXISTS uq_documento_sha256_por_requisito

ON sigd_doc.documento_adjunto(id_expediente_requisito, sha256_hash);
...
COMMENT ON TABLE sigd_doc.expediente_formulario_respuesta IS

'Respuestas del formulario asociadas 1:1 al expediente y a su versión exacta.';

COMMENT ON TABLE sigd_doc.expediente IS

'Instancia concreta del trámite. BORRADOR no equivale a radicación administrativa.';

COMMENT ON COLUMN sigd_doc.expediente_formulario_respuesta.payload_respuestas IS

'Respuestas del solicitante en JSONB; la validación completa contra la versión exacta de FORMULARIO_VERSION corresponde a la capa de aplicación.';
...
'Hash SHA-256 del contenido binario real; se calcula en la capa de aplicación y queda inmutable. La deduplicación (uq_documento_sha256_por_requisito) es por requisito, no global: el mismo contenido puede repetirse en requisitos distintos bajo confirmación.';
...
-- *CORRECCIÓN v6.2: la validación de duplicados ahora se limita al mismo*

-- *id_expediente_requisito, en lugar de comparar contra toda la tabla. Esto*

-- *permite que el mismo contenido (mismo sha256_hash) se registre en*

-- *requisitos distintos, tal como exige la regla de negocio.*
...
SELECT 'tipo_documento_activo_default_true' AS prueba,
       EXISTS (SELECT 1 FROM pg_attribute a JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum
               WHERE a.attrelid='sigd_doc.tipo_documento'::regclass AND a.attname='activo'
                 AND pg_get_expr(d.adbin,d.adrelid)='true') AS ok;
SELECT 'tupa_calificacion_not_null' AS prueba,
       EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid='sigd_doc.tipo_tramite_tupa'::regclass
               AND attname='calificacion_administrativa' AND attnotnull) AS ok;
SELECT 'expediente_formulario_respuesta' AS prueba,
       to_regclass('sigd_doc.expediente_formulario_respuesta') IS NOT NULL AS ok;
COMMIT;
